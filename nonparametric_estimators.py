...
class SurvivalFunctionEstimator(BaseEstimator):
    ...
    def predict_proba(self, time, current = None):
        ...
        if current == True:
            if self.prob_[-1] > 0 and extends.any():
                time = time[time < self.unique_time_[-1]]
                print('time shape: ',time.shape)
                extends = extends[extends == False]
                if len(extends) != len(time):
                    extends = extends[0:len(time)]
                    print('extends changed, new shape: ', extends.shape)
                print("time changed")
