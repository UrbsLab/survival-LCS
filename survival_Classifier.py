...
class Classifier: #this script is for an INDIVIDUAL CLASSIFIER
    def __init__(self,model):
        ...
        self.matchingInstances = [] #for each rule, a list of instance indicies that were correctly covered
...
### Creating a continuous event range (endpoint):           
        if self.eventStatus == 1: #if the event occured
            ...
            if eventTime >= (model.env.formatData.eventList[1] - int(rangeRadius)): #temporary workaround for a range error (eventtime for censored instances is greater than the maxTime - rangeRadius).
                Low = float(self.eventTime + 1)
                High = float(model.env.formatData.eventList[1])
                self.eventInterval = [Low,High]
                self.setEventProb(model, model.env.formatData.eventRanked)
            else:
        ...
        for instance_index in range(num_instances): #for each instance in the environment 
            ...
            if self.match(model, state): #apply match function 
                ...
                if eventStatus == 1:
                    ...
                    #if (float(eventTime) < float(self.eventInterval[0])):
                        correct_count += 1
                       # self.updateCorrect()
                        self.updateError(model,eventTime,eventStatus)
                    #elif (float(eventTime) <= float(self.eventInterval[1]) and float(eventTime) >= float(self.eventInterval[0])):
                        #here need to get proportional error, how much to adjust the error by depending on "how incorrect" the rule is.
                        #try:
                            #propAdj = (eventTime - self.eventInterval[0])/(self.eventInterval[1]-self.eventInterval[0]) 
                            #self.updateCensoringError(propAdj) #give to new funct updateCensoring error, will multiply this number by 1
                       # except:
                         #   self.updateIncorrectError()                    
                    else:    
                        self.updateIncorrectError()
        self.accuracy = self.updateAccuracy(model) #updateError has now been called above
        #print('accuracy updated: ', self.accuracy)
            #print('Accuracy updated using accuracy function.')
 #       except:
 #           self.accuracy = (correct_count / match_count)   #keeping this here just in case
 #           print('Accuracy updated using correct and match counts.')
...
#----------------------------------------------------------------------------------------------------------------------------                              
    def setEventProb_old(self,model,eventRanked): #only considers instances with eventStatus = 1
        count = 0
        ref = 0
#         print self.eventRanked
#         print self.eventList
        if self.eventInterval[0] >= self.eventInterval[1]:
                self.event_RP = 0
        else:        
            while ref < len(eventRanked):
                if eventRanked[ref][1]==1:
                    ...
                    if eventRanked[ref][0] <= self.eventInterval[0]:
                        ...
                        #print("eventInterval[0]: ", self.eventInterval[0], "eventInterval[1]: ", self.eventInterval[1], "eventList[1]: ", model.env.formatData.eventList[1], "eventRanked[ref][0]: ",eventRanked[ref][0])
                    ref += 1
            self.event_RP = count/float(model.env.formatData.numTrainInstances)
            print('event_RP: ',self.event_RP)

#----------------------------------------------------------------------------------------------------------------------------
# setEventProb_new: Calculate the relative probability that an event time of an instance in the training data will fall withing the event range specified by this rule. 
#----------------------------------------------------------------------------------------------------------------------------                              
    def setEventProb(self,model,eventRanked): #only considers instances with eventStatus = 1
#         print self.eventRanked
#         print self.eventList
        if self.eventInterval[0] >= self.eventInterval[1]: #if the interval is 0, set the prob. to zero
            self.event_RP = 0
        elif self.eventInterval[1] > model.env.formatData.eventList[1]:
            self.event_RP = float((model.env.formatData.eventList[1]-self.eventInterval[0])/(model.env.formatData.eventList[1] - model.env.formatData.eventList[0]))
        elif self.eventInterval[0] < model.env.formatData.eventList[0]:
            self.event_RP = float((self.eventInterval[1]-model.env.formatData.eventList[0])/(model.env.formatData.eventList[1] - model.env.formatData.eventList[0]))  
        
        else:        
            self.event_RP = float((self.eventInterval[1]-self.eventInterval[0])/(model.env.formatData.eventList[1] - model.env.formatData.eventList[0]))
                        #print("eventInterval[0]: ", self.eventInterval[0], "eventInterval[1]: ", self.eventInterval[1], "eventList[1]: ", model.env.formatData.eventList[1], "eventRanked[ref][0]: ",eventRanked[ref][0])            
        if self.event_RP > 1:
            print("eventInterval[0]: ", self.eventInterval[0], "eventInterval[1]: ", self.eventInterval[1], model.env.formatData.eventList[0],model.env.formatData.eventList[1])
            self.event_RP = 1    
...
#        if self.coverDiff < 0:
#            print('self.coverDiff, eventRP and matchcover: ',self.coverDiff,self.event_RP, self.matchCover) 
...
#----------------------------------------------------------------------------------------------------------------------------
# updateCensoringError: adds a proportional amount of error (1) to errorSum for censored instances in [I] where the eventInterval INCLUDES the censoring time
#----------------------------------------------------------------------------------------------------------------------------                 
    def updateCensoringError(self,propAdj):        
#        if not self.epochComplete:
        self.errorSum += 1.0 * propAdj
        self.errorCount += 1
...
#New as of 12/10, copied over from pareto version: for explanation, see [here](https://github.com/alexa-woodward/surviving-heterogeneity/blob/master/docs/misc/Retooling%20Fitenss%20for%20Noisy%20Problems%20in%20an%20LCS.pdf)             
    def updateAccuracy(self,model):
        ...
        if maxAccuracy == 0: #if max accuracy is zero
            ...
            accComp_1 = self.accuracyComponent
        ...
        if self.accuracyComponent < 0:
            print('self.accuracy: ', self.accuracy, 'self.event_RP: ', self.event_RP, 'adjAccuracy: ', adjAccuracy, 'maxAccuracy: ', maxAccuracy, 'accComp_1: ', accComp_1)
...
#             print self.accuracyComponent
        ...
        if self.fitness < 0:
            ...
            print("accuracy component: ", self.accuracyComponent, "coverage: ",self.coverDiff)
...
#---------------------------------------------------------------------------------------------------------------------------- 
    def subsumes(self,model,cl):
        ...
        if self.eventInterval[0] >= cl.eventInterval[0] and self.eventInterval[1] <= cl.eventInterval[1]:
                if self.isSubsumer(model) and self.isMoreGeneral(model,cl):
                    return True
        return False
#----------------------------------------------------------------------------------------------------------------------------
