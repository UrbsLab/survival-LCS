...
class survivalLCS(BaseEstimator,ClassifierMixin):
    def __init__(self,learning_iterations=100000,N=1000,nu=1,chi=0.8,mu=0.04,upsilon=0.04,theta_GA=25,theta_del=20,theta_sub=20,
                 ...
                 track_accuracy_while_fit=True,random_state=None):
        '''
    ...
    def fit(self, X, y, z):
        ...
        while self.iterationCount < self.learning_iterations:
            ...
            if len(self.trackingAccuracy) != 0:
                ...
                accuracy = 0
           # print("accuracy in fit?: ", accuracy)
...
#-----------------------------------------------------------------------------------------------------------------
    def saveFinalMetrics(self):
        self.finalMetrics = [self.learning_iterations,self.timer.globalTime, self.timer.globalMatching,self.timer.globalCovering,
                             self.timer.globalCrossover, self.timer.globalMutation, self.timer.globalAT,self.timer.globalEK,
                             self.timer.globalInit, self.timer.globalAdd, self.timer.globalRuleCmp,self.timer.globalDeletion,
                             self.timer.globalSubsumption, self.timer.globalSelection, self.timer.globalEvaluation,copy.deepcopy(self.AT),copy.deepcopy(self.env),copy.deepcopy(self.population.popSet),self.preRCPop]


        #print("size of learning iterations: ",sys.getsizeof(self.learning_iterations))
...
#-----------------------------------------------------------------------------------------------------------------
    def pickle_model(self,filename=None,saveRCPop=False):
        if self.hasTrained and self.learning_iterations == self.iterationCount: # Check hasFit, and there is new stuff to pickle
            ...
            pickle.dump(finalMetricsCopy, outfile, protocol=pickle.HIGHEST_PROTOCOL)
            outfile.close()
...
#-----------------------------------------------------------------------------------------------------------------
    def plotfeatImp(self, outputPath, model_type, dataFeatures, dataHeaders, featImp, cens_prop=None):
        ...
        featImp_meds = featImp_data.median()
        ...
        if cens_prop != None:
            ...
            ax.set_title('Survival LCS Feature Importances')
            plt.savefig(outputPath+'/cens_'+str(cens_prop)+'/sLCS_featImp_'+ model_type + '.png')
            plt.show()
        else:
            fig, ax = plt.subplots(figsize=(10,7))
            featImp_data.boxplot(ax=ax)
            ax.set_title('Survival LCS Feature Importances')
            plt.savefig(outputPath+'/sLCS_featImp_'+ model_type + '.png')
            plt.show()
...
#-----------------------------------------------------------------------------------------------------------------
    def plotKM(self, outputPath, dataFeatures, dataEvents, dataHeaders, k, cens_prop = None, attSums = None): #k is how many attributes to plot, dataFeatures could be drawn from elsewhere
        ...
        if cens_prop == None and np.isnan(np.sum(dataFeatures)) == True:
            imputer = KNNImputer(n_neighbors=2, weights="uniform")
            imputedFeatures = imputer.fit_transform(dataFeatures)
        ...
        if cens_prop != None:
            ...
            for row in ax:
                for col in topFeat:
                    for snp in dataFeatures[col].unique():
                        mask_snp = dataFeatures[col] == snp
                        #print('snp: ', snp, 'mask_snp: ', mask_snp, 'dataEvents[cens][mask_snp]: ', dataEvents['cens'][mask_snp],'dataEvents[time][mask_snp]: ', dataEvents['time'][mask_snp])
            ...
            for row in ax:
                for col in topFeat:
                    for snp in dataFeatures[col].unique():
                        mask_snp = dataFeatures[col] == snp
                        #print('snp: ', snp, 'mask_snp: ', mask_snp, 'dataEvents[cens][mask_snp]: ', dataEvents['cens'][mask_snp],'dataEvents[tim$
                        time_snp, survival_prob_snp = kaplan_meier_estimator(dataEvents['cens'][mask_snp], dataEvents['time'][mask_snp])
