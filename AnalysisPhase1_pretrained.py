...
def main(argv):
    ...
    parser.add_argument('--cluster', dest='do_cluster', type=int, default=0)
    ...
    for cv in range(cv_count):
        ...
        train_dataEvents = np.core.records.fromarrays(dataEvents_train.transpose(),names='cens, time', formats = '?, <f8')
        test_dataEvents = np.core.records.fromarrays(dataEvents_test.transpose(),names='cens, time', formats = '?, <f8')

        
        

        cv_info.append(
            [train_data_features, train_data_times, train_data_statuses, train_instance_labels, train_group_labels, train_dataEvents,test_data_features,
             test_data_times, test_data_statuses, test_instance_labels, test_group_labels, test_dataEvents, use_inst_label, use_group_label,cv_header_save])
...
def submitClusterJobOld(cv,experiment_path,memory1,memory2):
