...
def job(experiment_path, at_height_factor):
    ...
    cv_count = phase1_pickle[10]
    cv_info = phase1_pickle[0]
    full_info = phase1_pickle[1]
    data_features = full_info[0]
    data_times = full_info[1]
    data_statuses = full_info[2]
    data_headers = full_info[3]
    full_instance_labels = full_info[4]
    full_group_labels = full_info[5]
    group_colors = full_info[6]
    ...
    g = seaborn.clustermap(AT_full_df, metric='euclidean', method='single', cmap='plasma')
    ...
    clusters, colors = cluster_tree.getSignificantClusters(p_value=0.05, sample_count=100, metric='euclidean',method='single', random_state=random_state)

    AT_distortions = []

    precomputed_distances = squareform(pdist(merged_AT,metric='euclidean'))
    silhouettes = []
    ...
    for cluster_count in reversed(range(1, len(clusters) + 1)):
        ...
        subclusters, colors = cluster_tree.getNSignificantClusters(cluster_count, p_value=0.05, sample_count=100,metric='euclidean', method='single',random_state=random_state)
        ...
        with open(experiment_path + '/Composite/at/atclusters/' + str(cluster_count) + '_clusters/labeledDataset.csv',mode='w') as file:
            writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(['Instance', 'Cluster'] + list(data_headers) + [time_label] + [status_label])
            for cluster in subclusters:
                ...
                for exp_instance in cluster:
                    ...
                    exp_times = data_times[exp_index]
                    writer.writerow([exp_instance, exp_color] + exp_state + [exp_times])
        file.close()
        ...
        with open(experiment_path + '/Composite/at/atclusters/' + str(cluster_count) + '_clusters/clusters.csv',mode='w') as file:
            ...
            for cluster in subclusters:
                ...
                writer.writerow(['Instance Label', 'True Group', ''] + list(data_headers) + [time_label] + [status_label])
                ...
                for exp_instance in cluster:
                    ...
                    exp_state = data_features[exp_index].tolist()
                    exp_times = data_times[exp_index]
                    writer.writerow([exp_instance, exp_group, ''] + exp_state + [exp_times])
                    ...
                    if exp_group in group_makeup:
                        ...
                        group_makeup[exp_group] = 1 / len(cluster)
                    if exp_times in class_makeup:
                        class_makeup[exp_times] += 1 / len(cluster)
                    else:
                        class_makeup[exp_times] = 1 / len(cluster)
