...
sys.path.append("/Users/harshbandhey/Local/Cedars/Urbslab/survival-LCS-Final")
...
[rereqs
pip3 install LCS-DIVE pandas matplotlib scikit-learn scikit-ExSTraCS skrebate fastcluster seaborn networkx pygame pytest-shutil eli5 scikit-survival
'''
...
homedir = "/Users/harshbandhey/Local/Cedars/Urbslab/survival-LCS-Final/pipeline"
    ...
    for i in range(0,len(models)):
        for j in range(0,len(nfeat)):
            for k in range(0,len(maf)):
                ...
                from survival_LCS_pipeline import survivalLCS
                survivalLCS = survivalLCS(g, mtype,d, m, o, e,brier_df,cox_brier_df, m0_path, m0_type, m1_path,m1_type,c = c, iterations = 100, cv = 3)
        ...
        if lcs_run == True:
            ...
            for i in range(1,len(dtype_list)):
                try:
                    ...
                    pass
            try:
                plt.savefig(homedir +'/'+'sim_lcs_output/' + '/brier_scores_' + str(models[i]) + '.png')
            except Exception:
                print(models)
            plt.close('all')
        else:
